...
import ItemDetailClient from "../../../../../menu/item/[id]/ItemDetailClient";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale, tenant, branchSlug, id } = await params;
  ...
  const categories = await getPublicMenu(tenant, branchSlug);
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/${locale}/${tenant}/b/${branchSlug}/menu/item/${id}`,
    },
    openGraph: {
      title: `${itemName} | ${tenantName}`,
      description: itemDesc,
...
export default async function BranchItemDetailPage({ params }: Props) {
  const { locale, tenant, branchSlug, id } = await params;
